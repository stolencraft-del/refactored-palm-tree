...
async def txt_handler(bot: Client, m: Message):
    ...
    try:
        for i in range(arg-1, len(links)):  # Iterate over each link
            ...
            # Create final name (use PRENAME if provided, otherwise fallback to CREDIT)
            try:
                if "," in raw_text3:
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            # Handle Spayee links
                ...
                # Extract HLS key from URL if present
                if "HLS_KEY=" in url:
                    spayee_key = url.split("HLS_KEY=")[1].split("&")[0] if "&" in url.split("HLS_KEY=")[1] else url.split("HLS_KEY=")[1]
                    url = url.split("HLS_KEY=")[0].rstrip("?&")
                else:
                    spayee_key = None
            ...
            # ----------- 1. DRM LINKS (Team‑JNC DRM API) -----------
            elif "https://cpvod.testbook.com/" in url:
                url = url.replace("https://cpvod.testbook.com/", "https://media-cdn.classplusapp.com/drm/")
                api_url = f"https://team-jnc-ba95987c225e.herokuapp.com/api?url={url}"
            ...
            # ----------- 2. NON‑DRM SIGNED LINKS (Team‑JNC N‑DRM API) -----------
              ...
              or "media-cdn-a.classplusapp.com" in url:
                url = requests.get(f'https://team-jnc-n-drm.vercel.app/api?url={url}').json()["url"]

            # ----------- 3. OTHER SOURCES REMAIN UNCHANGED -----------
            ...
            elif 'encrypted.m' in url:              # (old appxkey logic unchanged)
                appxkey = url.split('*')[1]
                url = url.split('*')[0]

            # ----------- 4. yt-dlp FORMAT LOGIC -----------
            if "youtu" in url:
